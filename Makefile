...
define Package/php5-mod-openssl/postinst
#!/bin/sh
[ -n "$$IPKG_INSTROOT" ] || {
	if [ -e "/etc/php5/openssl.ini" ] && [ -e "/etc/php5/15_openssl.ini" ]; then
		echo "Renaming PHP ini file '/etc/php5/openssl.ini' -> '/etc/php5/15_openssl.ini'"
		mv /etc/php5/openssl.ini /etc/php5/15_openssl.ini
	fi
}
endef

define Package/php5-mod-mysqlnd/postinst
#!/bin/sh
[ -n "$$IPKG_INSTROOT" ] || {
	if [ -e "/etc/php5/mysqlnd.ini" ] && [ -e "/etc/php5/20_mysqlnd.ini" ]; then
		echo "Renaming PHP ini file '/etc/php5/mysqlnd.ini' -> '/etc/php5/20_mysqlnd.ini'"
		mv /etc/php5/mysqlnd.ini /etc/php5/20_mysqlnd.ini
	fi
}
endef
